    ...
    export default {
        ...
        methods: {
            add() {
                ...
                setTimeout(async() => {
                    const res = await
                        fetch(`https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=36ae75ab82703251b7a767a4812c40f8d6e501bfe64835d3b6b2b17efee10cae`)
            ...
            handleRemove(itemToRemove) {
                this.tickers = this.tickers.filter(item => item !== itemToRemove)
            },
            normalizeGraph() {
